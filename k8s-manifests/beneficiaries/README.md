# Kubernetes/OpenShift Manifests for beneficiaries

Generated from Score specification: `score-beneficiaries-generated.yaml`
Platform: **kubernetes**
...
kubectl apply -f k8s-manifests/beneficiaries/beneficiaries/

# Or for OpenShift
oc apply -f k8s-manifests/beneficiaries/beneficiaries/
```
...
kubectl wait --for=condition=ready pod -l component=redis -n default --timeout=300s
...
kubectl wait --for=condition=available deployment/beneficiaries -n default --timeout=300s
...
kubectl logs -f deployment/beneficiaries -n default

# Check database logs
kubectl logs -f statefulset/beneficiaries-db -n default

# Check Redis logs
kubectl logs -f statefulset/beneficiaries-redis -n default
```
...
```bash
kubectl port-forward svc/beneficiaries 8080:80 -n default
...
```
https://beneficiaries.example.com
```
...
```bash
kubectl edit configmap beneficiaries-config -n default
# Restart pods to pick up changes
kubectl rollout restart deployment/beneficiaries -n default
```

### Update Secrets

```bash
kubectl edit secret beneficiaries-secret -n default
# Restart pods to pick up changes
kubectl rollout restart deployment/beneficiaries -n default
```
...
```bash
kubectl scale deployment/beneficiaries --replicas=5 -n default
```
...
```bash
# Watch HPA
kubectl get hpa beneficiaries -n default --watch
...
kubectl delete -f k8s-manifests/beneficiaries/beneficiaries/
