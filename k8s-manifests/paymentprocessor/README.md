# Kubernetes/OpenShift Manifests for paymentprocessor

Generated from Score specification: `score-paymentprocessor-generated.yaml`
Platform: **kubernetes**
...
kubectl apply -f k8s-manifests/paymentprocessor/paymentprocessor/

# Or for OpenShift
oc apply -f k8s-manifests/paymentprocessor/paymentprocessor/
```
...
kubectl wait --for=condition=available deployment/paymentprocessor -n default --timeout=300s
...
kubectl logs -f deployment/paymentprocessor -n default

# Check database logs
kubectl logs -f statefulset/paymentprocessor-db -n default
```
...
```bash
kubectl port-forward svc/paymentprocessor 8081:80 -n default
...
```
https://paymentprocessor.example.com
```
...
```bash
kubectl edit configmap paymentprocessor-config -n default
# Restart pods to pick up changes
kubectl rollout restart deployment/paymentprocessor -n default
```

### Update Secrets

```bash
kubectl edit secret paymentprocessor-secret -n default
# Restart pods to pick up changes
kubectl rollout restart deployment/paymentprocessor -n default
```
...
```bash
kubectl scale deployment/paymentprocessor --replicas=5 -n default
```
...
```bash
# Watch HPA
kubectl get hpa paymentprocessor -n default --watch
...
kubectl delete -f k8s-manifests/paymentprocessor/paymentprocessor/
